...
class Validator:
	def __init__(
		...
		####
		self.reg_lrn=re.compile(r"^([!-?A-^`-~]*)__([0-9a-f]+)__([!-?A-^`-~]+)__([!-?A-^`-~]*)$")

		self.reg_prefix_part=re.compile(r"^[!-?A-^`-~]*$")
		self.reg_id_part=re.compile(r"^[0-9a-f]+$")
		self.reg_segmental_part=re.compile(r"^(?:(\([0-9FRN,]*\))(?:,(?!$)|$))+$")
		self.reg_suffix_part=re.compile(r"^(?:((?:[a-zA-Z0-9]+:){0,1})\[([!-?A-Z\\^`-~]*)\](?:,(?!$)|$))+$")

		self.reg_segment=re.compile(r"^\(([0-9]+),([0-9]+),([FRN]),([0-9]+),([0-9]+)\)$")
		#self.reg_comment=re.compile(r"^[([!-?A-Z\\^`-~]*)]$")
		self.reg_extension=re.compile(r"^([A-Za-z0-9]+):\[([!-?A-Z\\^`-~]*)\]$")
		#####

		self.widths=self._widths_from_read_name(initial_read_name)

	def validate(self,read_name):
		if self.reg_lrn.match(read_name) is None:
			self.report_error(
					read_name=read_name,
					error_name="wrong_read_name_structure",
					message="'{}' is not matched".format(self.reg_lrn),
				)
		else:
			parts=read_name.split("__")

			if self.reg_prefix_part.match(parts[0]) is None:
				self.report_error(
						read_name=read_name,
						error_name="wrong_prefix_part",
						message="'{}' is not matched".format(self.reg_prefix_part),
					)

			if self.reg_id_part.match(parts[1]) is None:
				self.report_error(
						read_name=read_name,
						error_name="wrong_id_part",
						message="'{}' is not matched".format(self.reg_id_part),
					)

			if self.reg_segmental_part.match(parts[2]) is None:
				self.report_error(
						read_name=read_name,
						error_name="wrong_segmental_part",
						message="'{}' is not matched".format(self.reg_segmental_part),
					)

			if self.reg_suffix_part.match(parts[3]) is None:
				self.report_error(
						read_name=read_name,
						error_name="wrong_suffix_part",
						message="'{}' is not matched".format(self.reg_suffix_part),
					)
			...
			if self.widths != widths:
				self.report_error(
						read_name=read_name,
						error_name="wrong_widths",
						message="Integers have different widths ({} vs. {}).".format(widths, self.widths),
						warning=True,
					)
