...
class Bam:
	...
	def _vector_of_categories(srs, read_tuple_name, parts):
		...
		for q in range(len(srs)):
			...
			#####
			if srs[q]["U"] > 0 and srs[q]["u"] == 0 and srs[q]["m"] == 0 and srs[q]["w"] == 0 and len(
					srs[q]["M"]) == 0:
				assert vec[q] == "x", str((q, srs[q]))
				vec[q] = "U"
	...
	def create_et(self):
		...
		with (gzip.open(self._es_fn, "tr") if self.compress_intermediate_files else open(self._es_fn, "r")) as es_fo:
			with (
					gzip.open(self._et_fn, "tw+") if self.compress_intermediate_files else open(self._et_fn,
						"w+")) as et_fo:
				self.es2et(
					es_fo=es_fo,
					et_fo=et_fo,
	...
	def et2roc(et_fo, roc_fo):
		...
		for line in stats_dicts:
			numbers = [line["M"], line["w"], line["m"], line["P"], line["U"], line["u"], line["T"], line["t"],
				line["x"]]
	...
	def create_html(self):
		...
		with open(self._html_fn, "w+") as html:
			...
			for (lvalue, rvalue) in [
				...
				eval_info.append(
					'<tr><td style="font-weight:bold">{}:</td><td style="text-align:left">{}</td></tr>'.format(lvalue,
						rvalue))
