...
def add_source(sample):
	if len(samples()) == 0:
		rnftools.utils.error("No sample defined",program="RNFtools",subprogram="MIShmash",exception=ValueError)
...
def sample(name,reads_in_tuple):
	if name in [sample.get_name() for sample in __SAMPLES__]:
		rnftools.utils.error("More samples have the same name. Each sample must have a unique name.",program="RNFtools",subprogram="MIShmash",exception=ValueError)
