...
class Segment:
    ...
    def __init__(self, genome_id=0, chr_id=0, direction="N", left=0, right=0):
        """
		Args:
        ...
        if not (self.right == 0 or self.right >= self.left):
            smbl.messages.error(
                "Leftmost coordinate cannot be higher than rightmost coordinate.", program="RNFtools",
                subprogram="RNF format", exception=ValueError
            )
